...
# fmt: off
_GTLD_INCLUDE_SUBDOMAINS = {b'search', b'gle', b'new', b'hangout', b'foo', b'page', b'dev', b'gmail', b'chrome', b'google', b'app', b'insurance', b'bank', b'android', b'youtube', b'meet', b'play'}  # noqa: E501
...
# leak cache outside the package, it was not the case before
_in_hsts_preload_cache = LRUCache(1024)
...
class InHstsPreload:
    ...
    __slots__ = ["result", "host", "jump_iterator", "next_jump", "current_label"]

    def __init__(self, host: typing.AnyStr):
    ...
    def _start(self) -> None:
        labels = self.host.split(b".")
        # Fast-branch for gTLDs that are registered to preload all sub-domains.
        if labels[-1] in _GTLD_INCLUDE_SUBDOMAINS:
            self._set_result(True)
        else:
            self.result = None
            self.jump_iterator = InHstsPreload._iter_jumps(labels)
            self._prepare_next_read()

    def _set_result(self, result: bool) -> None:
        # see also CachedInHstsPreload._set_result
        self.result = result

    def get_offset_size(self) -> typing.Tuple[int, int]:
        offset, size, self.current_label = self.next_jump
        self.next_jump = None
        return offset, size

    def read(self, data: bytes) -> None:
        for is_leaf, include_subdomains, ent_label in InHstsPreload._iter_entries(data):
            # We found a potential leaf
            if is_leaf:
                if ent_label == self.host:
                    self._set_result(True)
                    return
                if include_subdomains and self.host.endswith(b"." + ent_label):
                    self._set_result(True)
                    return
            # Continue traversing as we're not at a leaf.
            elif self.current_label == ent_label:
                break
        else:
    ...
    def _prepare_next_read(self) -> None:
        jump = next(self.jump_iterator, None)
        if jump is None:
    ...
    @staticmethod
    def _iter_jumps(
        labels: typing.List[bytes],
    ) -> typing.Iterable[typing.Tuple[int, int, bytes]]:
        for layer, label in enumerate(labels[::-1]):
            # None of our layers are greater than 4 deep.
            if layer > 3:
                break
            # Read the jump table for the layer and label
            yield _JUMPTABLE[layer][_crc8(label)] + (label,)
...
class CachedInHstsPreload(InHstsPreload):

    __slots__ = ["cache"]

    def __init__(self, host: typing.AnyStr, cache: LRUCache):
        self.cache = cache
        super().__init__(host)

    def _start(self) -> None:
        self.result = self.cache[self.host]
        if self.result is None:
            super()._start()

    def _set_result(self, result: bool) -> None:
        self.cache[self.host] = result
        # small optimization: don't call super
        self.result = result


def _open_data() -> typing.BinaryIO:
    return open_pkg_binary("hstspreload.bin")


def _load_data(data: bytearray, f: typing.BinaryIO, offset: int, size: int) -> None:
    f.seek(offset)
    f.readinto(data)


def in_hsts_preload(host: typing.AnyStr) -> bool:
    h = CachedInHstsPreload(host, _in_hsts_preload_cache)
    if h.result is None:
        with _open_data() as f:
            while h.result is None:
                offset, size = h.get_offset_size()
                data = bytearray(size)
                _load_data(data, f, offset, size)
                h.read(data)
    return h.result


async def in_hsts_preload_async(
    host: typing.AnyStr,
    ...
    if h.result is None:
        ...
        f = await loop.run_in_executor(executor, _open_data)
        try:
            while h.result is None:
                offset, size = h.get_offset_size()
                data = bytearray(size)
                await loop.run_in_executor(
                    executor, lambda: _load_data(data, f, offset, size)
                )
                h.read(data)
        finally:
            await loop.run_in_executor(executor, lambda: f.close())
    return h.result
